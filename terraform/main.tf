terraform {
  backend "s3" {
    ...
    key          = "education-platform/terraform.tfstate"
    region       = "us-west-2"
    use_lockfile = true
  }
}
...
# the comment that used to be here) was declared but never actually
# referenced by any resource in this repo. Dropped as dead weight; add it
# back properly (with its own OIDC-based auth) if/when a real ACM+CloudFront
# setup needs it.
