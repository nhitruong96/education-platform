...
data "aws_iam_policy_document" "github_actions_assume_role" {
  statement {
    ...
    # branch -- personal nhitruong96 repos use "main", not "master"), not
    # a bare repo:* wildcard.
    condition {
      # Two values because GitHub is migrating the `sub` claim to an
      # "immutable" form that embeds numeric owner and repo IDs, e.g.
      #   repo:nhitruong96@71067517/education-platform@1342417979:ref:refs/heads/main
      # instead of the classic
      #   repo:nhitruong96/education-platform:ref:refs/heads/main
      ...
      # only the numeric IDs, anchored after "nhitruong96@" and "/education-platform@".
      test     = "StringLike"
...
resource "aws_iam_role" "github_actions_education-platform" {
  name               = "github_actions_education-platform"
  assume_role_policy = data.aws_iam_policy_document.github_actions_assume_role.json
}

# ecr/ec2/route53 statements mirror the core repo's terraform/iam/policies/
# terraform_education-platform_policy.tf (granted to the old static-key
# IAM user) -- kept self-contained here rather than referencing that
# module, since the point of this migration is per-repo terraform
# ownership. The s3 statements are new: that old policy never needed
# bucket access because Jenkins ran this repo's terraform against local
# state, not the shared S3 backend this migration moves it onto.
resource "aws_iam_role_policy" "github_actions_education-platform_policy" {
  name = "github_actions_education-platform_policy"
  role = aws_iam_role.github_actions_education-platform.id
    ...
    "Statement": [
        {
            ...
            "Resource": "arn:aws:iam::975050308029:role/github_actions_education-platform"
        },
        {
            "Effect": "Allow",
            "Action": [
            ...
            "Resource": "arn:aws:ecr:${var.region}:975050308029:repository/education-platform"
        },
        {
            "Effect": "Allow",
            "Action": [
        ...
        },
        {
            ...
            "Resource": "arn:aws:s3:::prod-levantine-terraform-bucket/education-platform/*"
        },
        {
            "Effect": "Allow",
            "Action": [
